...
public class SQLiteRSSHelper extends SQLiteOpenHelper {
    ...
    public long insertItem(String title, String pubDate, String description, String link) {
        ...
        // Criar Content values e inserir valores
        ...
        cv.put(ITEM_UNREAD, 1);

        SQLiteDatabase dbWrite = db.getWritableDatabase();
        return dbWrite.insert(DATABASE_TABLE,null,cv);
    ...
    public ItemRSS getItemRSS(String link) throws SQLException {
        ...
        Cursor cursor = dbRead.query(DATABASE_TABLE, columns, ITEM_LINK + "=?", new String[]{link}, null, null, null);

        if(cursor.moveToFirst()){
            ItemRSS itemRSS = new ItemRSS(cursor.getString(cursor.getColumnIndexOrThrow(ITEM_TITLE)),
    ...
    public Cursor getItems() throws SQLException {
        ...
        SQLiteDatabase dbRead = db.getReadableDatabase();
        //selecionar itens que não foram lidos
        Cursor cursor = dbRead.query(DATABASE_TABLE, columns ,ITEM_UNREAD + "= 1",null,null,null, null);
        return cursor;
    ...
    public boolean markAsRead(String link) {
        SQLiteDatabase dbWrite = db.getWritableDatabase();
        ContentValues cv = new ContentValues();
        cv.put(ITEM_UNREAD , 0);

        int linhas_alteradas = dbWrite.update(DATABASE_TABLE,cv,ITEM_LINK + "=?", new String[] {link});

        // teste para ver se algo estranho acontece
        Log.d("TESTE","MARQUEI");
